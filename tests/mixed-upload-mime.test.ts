...
test("a generic MIME resume PDF beside an image is still indexed as a PDF", () => {
  const files = uploadResponsesToAttachments([
    {
      name: "scene.png",
      ufsUrl: "https://example.com/scene.png",
      type: "image/png",
      size: 500,
    },
    {
      name: "Shubhojeet_Bera_Resume.pdf",
      ufsUrl: "https://example.com/resume.pdf",
  ...
  const content = buildMultimodalContent(
    "Ye dono attachment mein koi similarity hai kya?",
    files,
  );
  assert.ok(Array.isArray(content));
  assert.equal(content.filter((part) => part.type === "image_url").length, 1);
  assert.equal(
    selectDocumentAttachmentsForTurn([{ attachments: files }], false).length,
    1,
  );
});

test("generic MIME for a document without a supported extension stays generic", () => {
  const [file] = uploadResponsesToAttachments([
    {
      name: "unknown.bin",
      ufsUrl: "https://example.com/unknown.bin",
      type: "application/octet-stream",
    },
  ]);
...
test("markdown text MIME on a .txt file is accepted for inline and indexed paths", () => {
  const [file] = uploadResponsesToAttachments([
    {
      name: "notes.txt",
      ufsUrl: "https://example.com/notes.txt",
      type: "text/markdown",
      size: 100,
    },
  ]);
  ...
  assert.equal(
    selectDocumentAttachmentsForTurn([{ attachments: [file] }], false).length,
    1,
  );
});
