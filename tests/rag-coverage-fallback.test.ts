...
import {
  getScopedDocumentCoverage,
  formatRetrievedContext,
} from "@/lib/rag/retrieval/context";
...
test("missed query can use bounded owner-scoped coverage samples, not fake retrieval hits", async () => {
  ...
  Object.defineProperty(prisma, "$queryRaw", {
    ...
    value: async (query: unknown) => {
      ...
      return [
        {
          id: "pdf-id:0",
          content: "Software engineering experience",
  ...
  try {
    const candidates = await getScopedDocumentCoverage(
      "owner-1",
      "conversation-1",
      ["pdf-id"],
    );
...
test("coverage rejects foreign or empty rows and never samples a document without its own chunk", async () => {
  ...
  Object.defineProperty(prisma, "$queryRaw", {
    ...
    value: async () => [
      {
        id: "foreign",
        content: "Private other person's file",
  ...
  try {
    assert.deepEqual(
      await getScopedDocumentCoverage("owner-1", "conversation-1", ["pdf-id"]),
      [],
    );
    assert.deepEqual(
      await getScopedDocumentCoverage("owner-1", "conversation-1", []),
      [],
    );
  } finally {
    Object.defineProperty(prisma, "$queryRaw", {
...
test("coverage requests two samples for each scoped document, even past five documents", async () => {
  ...
  Object.defineProperty(prisma, "$queryRaw", {
    configurable: true,
    value: async (query: unknown, ...values: unknown[]) => {
      captured = { query, values };
      return [];
    },
  });
  try {
    await getScopedDocumentCoverage(
      "owner-1",
      "conversation-1",
      Array.from({ length: 7 }, (_, i) => `pdf-${i}`),
    );
    const sql = Array.from(
      (captured as { query: TemplateStringsArray }).query,
    ).join(" ");
...
test("coverage returns a sample for each of seven indexed documents", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      new Set(samples.map((sample) => sample.metadata.attachmentId)),
      new Set(ids),
    );
    assert.equal(
      formatRetrievedContext(samples, "coverage").usedAttachmentIds.length,
      ids.length,
    );
  } finally {
    Object.defineProperty(prisma, "$queryRaw", {
