...
function NotSignedInView({ onGoHome }: { onGoHome: () => void }) {
  return (
    <div className="flex min-h-screen flex-col">
      <ChatHeader onNewChat={onGoHome} />
...
function ApplyChangesPanel({
  ...
  actionLabel: string;
  comparison: { hasChanges: boolean; hasRemovals: boolean; hasAdditions: boolean };
  isApplying: boolean;
    ...
    <Card className="h-fit border-border/60 bg-card shadow-none">
      <CardContent className="space-y-5 p-6">
        <div className="space-y-2">
          ...
          <p className="text-sm leading-6 text-muted-foreground">{actionDescription}</p>
        </div>
...
function ServiceCard({
  ...
  service: { id: GoogleWorkspaceServiceId; name: string; summary: string; levels: { id: string; label: string }[] };
  ServiceIcon: ComponentType<SVGProps<SVGSVGElement>>;
  currentLevel: { id: string; label: string };
  selectedLevel: { id: string; label: string; description: string };
  isChanged: boolean;
  selectedLevels: Record<string, string>;
  handleLevelChange: (serviceId: GoogleWorkspaceServiceId, levelId: string) => void;
}) {
  return (
    <Card
      className={cn(
...
export function GoogleWorkspaceSettings() {
  ...
  if (!session?.user) {
    return <NotSignedInView onGoHome={handleGoHome} />;
  }

  return (
    <div className="flex min-h-screen flex-col">
      <ChatHeader onNewChat={handleGoHome} />
