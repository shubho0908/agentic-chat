  ...
  if (variant === STRING_ENUM.USER_MESSAGE) {
    return (
      <div className={cn("inline-block w-full", className)}>
        ...
        >
          <div className="relative flex items-center gap-3.5 p-3 pr-4 transition-all duration-300 cursor-pointer shadow-sm rounded-2xl bg-black/5 dark:bg-zinc-950 border border-black/5 dark:border-white/5 hover:bg-black/10 dark:hover:bg-[#2C2C2E]">
            <div className="relative size-[60px] flex-shrink-0 overflow-hidden rounded-[12px] bg-white dark:bg-zinc-950 flex items-center justify-center shadow-sm dark:shadow-none border border-black/5 dark:border-white/10">
              {metadata.image ? (
                <SafeImage
                  src={metadata.image}
                  alt={displayTitle}
