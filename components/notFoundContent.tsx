      ...
      <CardFooter className="flex flex-col gap-4 pt-6">
        <div className="flex flex-col sm:flex-row gap-3 w-full">
          ...
          >
            {session ? (
              <>
                <Plus className="size-4 shrink-0 group-hover:scale-110 transition-transform" />
                <span className="truncate">New Chat</span>
              </>
            ) : (
              <>
                <LogIn className="size-4 shrink-0 group-hover:scale-110 transition-transform" />
...
export function NotFoundClient() {
  ...
  return (
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-background p-4">
      ...
      <Card className="relative z-10 w-full max-w-2xl border-border/40 bg-card/80 dark:bg-card/95 backdrop-blur-sm shadow-xl dark:shadow-2xl">
        <NotFoundHeader />
        <NotFoundContent />
        <NotFoundActions />
      </Card>
