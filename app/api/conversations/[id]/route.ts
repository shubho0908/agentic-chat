...
function toChatMessageForTokenUsage(message: {
  role: string;
  content: string;
  attachments?: MessageAttachment[];
}): Message {
  ...
  const role: Message["role"] = (
    Object.values(MessageRole) as string[]
...
export async function GET(
  ...
  try {
    ...
    const messageIds = originalMessages
      .slice(0, Math.min(originalMessages.length, limit))
      .map((m) => m.id);
    ...
    const transformedMessages = paginatedTree.map((msg) => {
      ...
      return {
        ...
        ...(includeVersions && {
          versions:
            msgWithAttachments.versions?.map((v) => ({
              ...v,
    ...
    try {
      ...
      tokenUsage = calculateTokenUsage(
        transformedMessages.map(toChatMessageForTokenUsage),
        model,
      );
    } catch (error) {
...
export async function PATCH(
  ...
  try {
    ...
    if (!isRecord(body)) {
      return errorResponse(
        API_ERROR_MESSAGES.INVALID_REQUEST_BODY,
        undefined,
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    ...
    if (title !== undefined) {
      if (typeof title !== "string" || !title.trim()) {
        return errorResponse(
          API_ERROR_MESSAGES.TITLE_REQUIRED,
          undefined,
          HTTP_STATUS.BAD_REQUEST,
        );
      }
      ...
      if (
        trimmedTitle.length > VALIDATION_LIMITS.CONVERSATION_TITLE_MAX_LENGTH
      ) {
        return errorResponse(
          API_ERROR_MESSAGES.TITLE_TOO_LONG,
          undefined,
          HTTP_STATUS.BAD_REQUEST,
        );
      }
    ...
    if (activeBranchId !== undefined) {
      if (
        activeBranchId !== null &&
        (typeof activeBranchId !== "string" ||
          activeBranchId.length === 0 ||
          activeBranchId.length > 255)
      ) {
        return errorResponse(
          API_ERROR_MESSAGES.INVALID_REQUEST_BODY,
          undefined,
